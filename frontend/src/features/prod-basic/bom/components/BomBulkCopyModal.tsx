...
interface BomBulkCopyModalProps {
  ...
  sourceBoms: BomResponse[]
  allItems: ItemResponse[]
  onConfirm: (targetItemIds: number[], mode: BomCopyModeType) => Promise<void>
  onClose: () => void
  isLoading: boolean
}

const BomBulkCopyModal = ({
  sourceItem,
  sourceBoms,
  allItems,
  onConfirm,
  onClose,
  isLoading,
}: BomBulkCopyModalProps) => {
  const [search, setSearch] = useState('')
  const [selectedIds, setSelectedIds] = useState<Set<number>>(new Set())
  const [mode, setMode] = useState<BomCopyModeType>(BomCopyMode.REPLACE)
  ...
  const filteredItems = useMemo(() => {
    ...
    if (!query) return candidateItems
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="flex max-h-[90vh] w-full max-w-xl flex-col rounded-xl bg-[var(--surface)] shadow-xl">
        <div className="flex items-center justify-between border-b border-[var(--border)] px-5 py-4">
          <h2 className="text-base font-semibold text-[var(--text-strong)]">BOM 일괄 적용</h2>
          <button
            type="button"
            onClick={onClose}
            disabled={isLoading}
        ...
        <div className="border-b border-[var(--border)] px-5 py-3">
          ...
          {mode === BomCopyMode.REPLACE && (
            <div className="mt-2 flex items-start gap-1.5 rounded-md bg-[var(--warning-soft,#fef9c3)] px-3 py-2 text-xs text-[var(--warning,#92400e)]">
              ...
              <span>대상 품목의 기존 BOM은 이력으로 보존되고 새 버전 BOM이 생성됩니다.</span>
            </div>
          )}
        </div>
...
export default BomBulkCopyModal
